...
from svtopovz.bed_plotter import (
    annotate_bed_records,
    add_box,
    add_strand,
    add_title,
...
class TestBedPlotter:
    ...
    def test_annotate_bed_records_none(self, mock_region, mock_ax):
        """Test annotate_bed_records with None bed_records"""
        annotate_bed_records(None, mock_region, mock_ax)
        # Should return early without error

    def test_annotate_bed_records_no_overlap(self, mock_region, mock_ax):
        """Test annotate_bed_records with no overlapping records"""
        bed_records = {"chr2": []}  # Different chromosome
        annotate_bed_records(bed_records, mock_region, mock_ax)
        # Should handle gracefully

    def test_annotate_bed_records_with_records(self, mock_region, mock_ax, sample_bed_records):
        """Test annotate_bed_records with valid bed records"""
        annotate_bed_records(sample_bed_records, mock_region, mock_ax)
    ...
    def test_add_strand_small_fraction(self, mock_ax):
        ...
        # Should add at least one strand indicator due to minimum count
    ...
    def test_add_strand_minimum_count(self, mock_ax):
        """Test add_strand ensures minimum count of 1"""
        bed_record = BedRecord(start=1000, end=1500, strand="+", title="test")
        strand_coords = []
        add_strand(bed_record, 500, 0.001, 100, 0.5, strand_coords, mock_ax)
        # Should add at least one strand indicator
        assert len(strand_coords) >= 1

    def test_annotate_bed_records_fixed_positions(self, mock_region, mock_ax, sample_bed_records):
        """Test annotate_bed_records uses fixed annotation positions"""
        annotate_bed_records(sample_bed_records, mock_region, mock_ax)
        # Should use fixed annotation_top=0.4 and annotation_bottom=0.2
        # This is different from annotation_plotter which uses level-based positioning
        assert len(mock_ax.patches) > 0

    def test_add_strand_arrow_count_calculation(self, mock_ax):
        """Test add_strand arrow count calculation"""
        bed_record = BedRecord(start=1000, end=1500, strand="+", title="test")
        strand_coords = []
        
        # Test with fraction that should give 0 arrows
        add_strand(bed_record, 500, 0.001, 100, 0.5, strand_coords, mock_ax)
        # Should still add at least one due to minimum count
        assert len(strand_coords) >= 1
        
        # Test with fraction that should give multiple arrows
        strand_coords.clear()
        add_strand(bed_record, 500, 0.5, 100, 0.5, strand_coords, mock_ax)
        # Should add multiple strand indicators
        assert len(strand_coords) > 1 